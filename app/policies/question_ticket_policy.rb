  ...
  def attributes_for_deep_search
    if user.one_of_roles?(:content_manager, :operator, :service_responsible)
      PolicyAttributes.new(
        ...
        sql_include: [:responsible_users, ticket: { service: :responsible_users }, answers: :attachments],
        serialize: ['responsible_users', 'ticket.service.responsible_users', 'answers.attachments']
      )
    else
      PolicyAttributes.new(
