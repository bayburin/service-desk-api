...
module Api
  module V1
    RSpec.describe EventsQuery, type: :model do
      let!(:searches) { create_list(:search_result_event, 3) }
      let!(:deep_searches) { create_list(:deep_search_result_event, 4) }
      ...
      describe '#all_search_result_by' do
        let(:date) { Date.today }

        it 'return all search events' do
          expect(subject.all_search_result_by(date).count).to eq scope.count
        end

        it 'return events by specified date' do
          subject.all_search_result_by(date).each do |event|
