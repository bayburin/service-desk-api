  ...
  module V1
    module QuestionTickets
      RSpec.describe DraftState do
        let(:popularity) { 50 }
        let(:original) { create(:question_ticket, state: :published, popularity: popularity) }
        let!(:question) { create(:question_ticket, state: :draft, original: original, popularity: 0) }
        ...
        describe '#destroy' do
          ...
          it 'destroys question_ticket' do
