      ...
      RSpec.describe UpdatePublishedQuestion do
        ...
        describe '#update' do
          ...
          context 'when ticket has correction' do
            let(:correction) { create(:question_ticket, state: :draft, original: question) }
            ...
            it 'does not create a correction' do
              expect { subject.update(updated_attributes) }.not_to change { QuestionTicket.count }
            end
          ...
          it 'creates a new question' do
            expect { subject.update(updated_attributes) }.to change { QuestionTicket.count }.by(1)
          end
          ...
          it 'clones attributes from original question' do
            ...
            expect(subject.question_ticket.original_id).to eq question.id
          end

          it 'clones attributes from original ticket' do
            subject.update(updated_attributes)

            expect(subject.question_ticket.ticket.service_id).to eq ticket.service_id
            expect(subject.question_ticket.ticket.popularity).to eq ticket.popularity
            expect(subject.question_ticket.ticket.sla).to eq ticket.sla
          end

          it 'clones responsible_users' do
            subject.update(updated_attributes)

            expect(subject.question_ticket.ticket.responsible_users.pluck(:tn)).to eq responsible_users.pluck(:tn)
          end
