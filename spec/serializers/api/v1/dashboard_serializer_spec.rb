...
module Api
  module V1
    RSpec.describe DashboardSerializer, type: :model do
      ...
      let(:category_attributes) { Oj.load(Categories::CategoryBaseSerializer.new(categories.first).to_json) }
      let(:service_attributes) { Oj.load(Services::ServiceBaseSerializer.new(services.first).to_json) }
